...
export default class I18nx {
     ...
     * @param use_default_extensions There are two extensions built in: one for plural rules, one for number representation. Use `true` to use all, `'plural'` to use only plural rules, or `false` to disable all (not recommended)
     */
    constructor(public use_default_extensions: boolean | 'plural' = true) {
        this.data = {};
        this.stack = [];
        this.extensions = {};
    }
    ...
    add(lang: string) {
        ...
        if (this.use_default_extensions) {
            exts.push(mkpluralext(lang));
            if (this.use_default_extensions !== 'plural')
